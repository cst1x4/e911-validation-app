...
# Initialize a persistent session state to hold search results across interactions
...
with input_col:
    ...
    input_unit = st.text_input("Unit / Apt / Suite (Optional)", placeholder="e.g., Suite 300")
    
    st.markdown(" ")
    
    # 🔎 THE SEARCH BUTTON INTERACTIVE GATEWAY
    # Clicking this button commits the inputs to the state and forces evaluation
    if st.button("🔎 Execute Validation Search", type="primary", use_container_width=True):
        if input_street and input_zip:
            st.session_state.search_executed = True
            st.session_state.active_street = input_street
            st.session_state.active_zip = input_zip
            st.session_state.active_unit = input_unit
        else:
    ...
    # Execute Stage 1 evaluation only if a search has been committed
    if st.session_state.search_executed:
        clean_street_key = st.session_state.active_street.strip().lower()
...
with diagnostic_col:
    ...
    # Execute Stage 2 evaluation only if a search has been committed
    if st.session_state.search_executed:
        ...
        if active_zip_str in MOCK_USPS_ZIP_MATRIX:
            ...
            st.warning("⚠️ **USPS System Exception:** Entering a ZIP outside the index drops validation capabilities. Try `80202`, `80222`, or `80111` for a complete end-to-end sandbox walkthrough.")
    else:
